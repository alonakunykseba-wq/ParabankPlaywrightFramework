...
| **Test Environment Lifecycle** | Running tests locally required manual setup in Docker Desktop. CI pipelines required complex command-line docker starts and curl wait-loops to verify readiness. | Integrate **Testcontainers** to control the Parabank Docker container lifecycle programmatically in Java using a TestNG `ISuiteListener` and dynamic URL injection. | Parabank is automatically pulled, started on a dynamic port, and verified ready before tests run. Pipeline configuration is simplified, and developers need zero manual GUI configuration. |
---
